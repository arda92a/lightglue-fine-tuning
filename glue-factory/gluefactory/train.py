...
def training(rank, conf, output_dir, args):
    ...
    # Test dataset (same as training dataset by default)
    ...
    if args.overfit:
        ...
        # Try to get test loader if test split exists
        try:
            test_loader = test_dataset.get_data_loader("test")
        except Exception:
            test_loader = None
    if rank == 0:
        ...
        for it, data in enumerate(train_loader):
            ...
            if (
                ...
                if rank == 0:
                    ...
                    wandb.log({
                        **{f"val/{k}": v for k, v in results.items() if isinstance(v, (int, float))},
                        **{f"val/{k}": v for k, v in pr_metrics.items() if isinstance(v, (int, float))},
                    }, step=epoch)
    ...
    if rank == 0 and test_loader is not None:
        ...
        if best_cp_path.exists():
            best_cp = torch.load(str(best_cp_path), map_location=device, weights_only=False)
            if args.distributed:
                model.module.load_state_dict(best_cp["model"])
            else:
        ...
        if best_cp_path.exists():
            best_artifact = wandb.Artifact(
                name=f"model-best-{args.experiment}",
                type="model",
                description="Best model checkpoint (lowest validation loss)"
            )
            best_artifact.add_file(str(best_cp_path), name="checkpoint_best.tar")
            wandb.log_artifact(best_artifact)
            logger.info("Uploaded best model artifact")
        
        # Upload last model (find the latest checkpoint)
        import glob
        last_checkpoints = sorted(glob.glob(str(output_dir / "checkpoint_*.tar")))
        last_checkpoints = [cp for cp in last_checkpoints if "best" not in cp]
        if last_checkpoints:
            last_cp_path = last_checkpoints[-1]
            last_artifact = wandb.Artifact(
                name=f"model-last-{args.experiment}",
                type="model",
                description="Last epoch model checkpoint"
            )
            last_artifact.add_file(last_cp_path, name="checkpoint_last.tar")
            wandb.log_artifact(last_artifact)
            logger.info(f"Uploaded last model artifact: {Path(last_cp_path).name}")
